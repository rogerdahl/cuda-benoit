...
// __LINE__
// The advantage is the developers gets to use the inline function so they can
// debug
...
#define cutilSafeMalloc(mallocCall) \
  __cutilSafeMalloc((mallocCall), __FILE__, __LINE__)
...
inline void __cudaSafeCallNoSync(cudaError err, const char* file, const int line)
{
  if (cudaSuccess != err) {
    fprintf(
        stderr,
        "cudaSafeCallNoSync() Runtime API error in file <%s>, line %i : %s.\n",
...
inline void __cudaSafeCall(cudaError err, const char* file, const int line)
{
  if (cudaSuccess != err) {
    fprintf(
        stderr,
        "cudaSafeCall() Runtime API error in file <%s>, line %i : %s.\n", file,
...
inline void __cudaSafeThreadSync(const char* file, const int line)
{
  ...
  if (cudaSuccess != err) {
    fprintf(
        stderr,
        "cudaThreadSynchronize() Driver API error in file '%s' in line %i : "
        "%s.\n",
        file, line, cudaGetErrorString(err));
    exit(-1);
  }
...
inline void __cufftSafeCall(cufftResult err, const char* file, const int line)
{
  if (CUFFT_SUCCESS != err) {
    fprintf(
        stderr, "cufftSafeCall() CUFFT error in file <%s>, line %i.\n", file,
        line);
    exit(-1);
  }
}
    ...
    const char* errorMessage, const char* file, const int line)
{
  ...
  if (cudaSuccess != err) {
    fprintf(
        stderr,
        "cutilCheckMsg() CUTIL CUDA error: %s in file <%s>, line %i : %s.\n",
  ...
  if (cudaSuccess != err) {
    fprintf(
        stderr,
        "cutilCheckMsg cudaThreadSynchronize error: %s in file <%s>, line %i : "
        "%s.\n",
        errorMessage, file, line, cudaGetErrorString(err));
    exit(-1);
  }
...
inline void __cutilSafeMalloc(void* pointer, const char* file, const int line)
{
  if (!(pointer)) {
    fprintf(
        stderr, "cutilSafeMalloc host malloc failure in file <%s>, line %i\n",
        file, line);
    exit(-1);
  }
}
...
inline void cutilCudaCheckCtxLost(
    const char* errorMessage, const char* file, const int line)
{
  cudaError_t err = cudaGetLastError();
  if (cudaSuccess != err) {
  ...
  if (cudaSuccess != err) {
    fprintf(
        stderr, "CCUDA error: %s in file '%s' in line %i : %s.\n", errorMessage,
        file, line, cudaGetErrorString(err));
    exit(-1);
  }
}
...
inline bool cutilCudaCapabilities(int major_version, int minor_version)
{
  ...
  if (deviceProp.major >= major_version && deviceProp.minor >= minor_version) {
    printf(
        "> Compute SM %d.%d Device Detected\n", deviceProp.major,
        deviceProp.minor);
    printf("> Device %d: <%s>\n", dev, deviceProp.name);
    return true;
  }
