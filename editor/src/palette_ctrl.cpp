...
public:
	RGBSlider(TemporalPaletteCtrl* temporal_palette_ctrl, wxPoint pt, wxSize size, const wxString& name);
	// Event handlers.
...
RGBSlider::RGBSlider(TemporalPaletteCtrl* pal, wxPoint pt, wxSize size, const wxString& name)
: wxSlider(pal,
           -1,
           0,
           0, 255,
...
double TemporalPaletteCtrl::Random() {
  ...
  boost::variate_generator<BaseGenerator&, boost::uniform_real<> > uni(rng, uni_dist);
  return uni();
}
...
bool TemporalPaletteCtrl::Create(wxWindow *parent, wxWindowID id, const wxPoint& pos, const wxSize& size, const wxString& name) {
	// Create control.
	...
	slider_red_ = new RGBSlider(this, wxDefaultPosition, wxDefaultSize, L"slider_red_");
	...
	slider_green_ = new RGBSlider(this, wxDefaultPosition, wxDefaultSize, L"slider_green_");
	text_green_ = new wxTextCtrl(this, -1, L"", wxDefaultPosition, wxSize(40, -1));
	...
	slider_blue_ = new RGBSlider(this, wxDefaultPosition, wxDefaultSize, L"slider_blue_");
...
// Render slider.
void TemporalPaletteCtrl::RenderSlider(wxDC& dc, bool focus, const wxRect& rect, const Pos& pos, const Color& color) {
	// Set up points for the slider shape:
	// x   x
	// x   x
	//   x
	const u32 slider_square_height((int)((double)rect.GetHeight() * ((double)SLIDER_SQUARE_PERCENT / 100)));
	wxPoint points[5];
...
bool TemporalPaletteCtrl::HitTestSlider(const wxRect& rect, const Pos pos, const wxPoint& check) {
...
void TemporalPaletteCtrl::OnPaint(wxPaintEvent& event) {
	...
	wxBitmap bitmap_tmp(GetClientSize().GetWidth(), GetClientSize().GetHeight(), -1);
	wxMemoryDC dc;
	...
	u32 slider_height((int)((double)palette_height * ((double)SLIDER_PERCENT / 100)));
	...
	u32 temporal_strip_start(temporal_slider_start + slider_height + PALETTE_SPACE);
	...
	u32 count(0);
	SpatialKeys& spatial_keys(temporal_palette_.GetTopTemporalKey().GetSpatialKeys());
	for (SpatialKeys::reverse_iterator riter(spatial_keys.rbegin()); riter != spatial_keys.rend(); ++riter) {
		RenderSlider(dc, ++count == spatial_keys.size(),
		wxRect(0, palette_slider_start, width, slider_height),
		riter->GetPos(), riter->GetColor());
	}

	// Render palette strip.
	u32 x1(SLIDER_WIDTH / 2);
	u32 x2(width - SLIDER_WIDTH / 2);
	ColorArray color_array(temporal_palette_.GetColorArray(temporal_palette_.GetTopTemporalKey().GetSpatialKeys(), x2 - x1));
	count = 0;
	for (u32 x(x1); x < x2; ++x) {
		Color color(color_array[count++]);
		dc.SetPen(wxPen(wxColor(color.red_, color.green_, color.blue_), 1, wxSOLID));
  ...
  //
	...
	for (TemporalKeys::reverse_iterator riter(temporal_keys.rbegin()); riter != temporal_keys.rend(); ++riter) {
		RenderSlider(dc, ++count == temporal_keys.size(),
		wxRect(0, temporal_slider_start, width, slider_height),
		riter->GetPos(), Color(128, 128, 128));
	}
	...
	for (TemporalKeys::iterator iter = sorted_temporal_keys.begin(); ; ++iter) {
		...
		ColorArray arr1(temporal_palette_.GetColorArray(iter->GetSpatialKeys(), strip_height));
		ColorArray arr2(temporal_palette_.GetColorArray(next->GetSpatialKeys(), strip_height));
		// Get positions for current pair.
		u32 x1(iter->GetPos() * (GetClientSize().x - SLIDER_WIDTH) + (SLIDER_WIDTH / 2));
		u32 x2(next->GetPos() * (GetClientSize().x - SLIDER_WIDTH) + (SLIDER_WIDTH / 2));
		//
		for (u32 x(x1); x < x2; ++x) {
  ...
  // After SetData(), rgbdata is owned by image.
	...
	// Copy to screen.
	dc_paint.Blit(0, palette_start, GetClientSize().GetWidth(), GetClientSize().GetHeight() - palette_start, &dc, 0, palette_start);
}
...
void TemporalPaletteCtrl::OnLeftDown(wxMouseEvent& event) {
	...
	if (slider_area.Contains(event.GetX(), event.GetY())) {
		...
		SpatialKeys& spatial_keys(temporal_palette_.GetTopTemporalKey().GetSpatialKeys());
		for (SpatialKeys::iterator iter(spatial_keys.begin()); iter != spatial_keys.end(); ++iter) {
			if (HitTestSlider(slider_area, iter->GetPos(), wxPoint(event.GetX(), event.GetY()))) {
    ...
    // the new key is set to the interpolated color at the given location.
      ...
      if (!CheckSpatialKeyLimit(spatial_keys)) {
        return;
			...
			spatial_key.SetPos((double)event.GetX() / (GetClientSize().x - SLIDER_WIDTH));
	...
	if (temporal_slider_area.Contains(event.GetX(), event.GetY())) {
		TemporalKeys& temporal_keys(temporal_palette_.GetTemporalKeys());
		for (TemporalKeys::iterator iter(temporal_keys.begin()); iter != temporal_keys.end(); ++iter) {
      // If click was on an existing slider.
			if (HitTestSlider(temporal_slider_area, iter->GetPos(), wxPoint(event.GetX(), event.GetY()))) {
				// Move temporal_slider to top of list, so it is drawn in front, then start
    ...
    if (!temporal_slider_is_dragging_) {
      ...
      temporal_palette_.CreateTemporalKey(click_pos, closest_key->GetSpatialKeys());
      TemporalPaletteUpdated();
    }
  }
}
...
void TemporalPaletteCtrl::OnMotion(wxMouseEvent& event) {
	if (spatial_slider_is_dragging_) {
		...
		spatial_key_dragging_->SetPos((double)(event.GetX() - (int)slider_dragging_hold_) / (double)(GetClientSize().x - SLIDER_WIDTH));
    TemporalPaletteUpdated();
	}
	if (temporal_slider_is_dragging_) {
		// Move slider being dragged.
		temporal_key_dragging_->SetPos((double)(event.GetX() - (int)slider_dragging_hold_) / (double)(GetClientSize().x - SLIDER_WIDTH));
    TemporalPaletteUpdated();
	}
}
...
void TemporalPaletteCtrl::OnRightDown(wxMouseEvent& event) {
  ...
  //
	...
	if (slider_area.Contains(event.GetX(), event.GetY())) {
		SpatialKeys& spatial_keys(temporal_palette_.GetTopTemporalKey().GetSpatialKeys());
		for (SpatialKeys::iterator iter(spatial_keys.begin()); iter != spatial_keys.end(); ++iter) {
			if (HitTestSlider(wxRect(0, palette_slider_start, width, palette_slider_start + slider_height), iter->GetPos(),
...
ColorArray TemporalPaletteCtrl::GetColorArray(const Pos pos, const u32 num_colors) {
...
void TemporalPaletteCtrl::RandomPalette(u32 num_colors, const vector<double>& positions) {
...
bool TemporalPaletteCtrl::CheckTemporalKeyLimit() {
  if (temporal_palette_.GetTemporalKeys().size() == MAX_TEMPORAL_KEYS) {
    wxMessageBox(
      str(wformat(L"For performance reasons, the player is limited to %d "
      L"temporal keys per track") % MAX_TEMPORAL_KEYS),
...
bool TemporalPaletteCtrl::CheckSpatialKeyLimit(const SpatialKeys& spatial_keys) {
  if (spatial_keys.size() == MAX_SPATIAL_KEYS) {
    wxMessageBox(
      str(wformat(L"For performance reasons, the player is limited to %d "
      L"spatial keys per temporal key") % MAX_SPATIAL_KEYS),
      L"Limit reached", wxCentre, this);
    return false;
  }
  return true;
...
PaletteEvent::PaletteEvent(TemporalPaletteCtrl* temporal_palette_ctrl, wxEventType type)
