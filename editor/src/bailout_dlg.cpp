...
BailoutDialog::BailoutDialog(wxWindow* parent, wxWindowID id,
...
: wxDialog(parent, id, L"Bailout", position, size)
{
  ...
  warning_msg += L"A higher bailout value enhances detail in the fractal. However,\n";
  warning_msg += L"a higher bailout values also increases the fractal calculation\n";
  warning_msg += L"time and makes it less likely that the player can rendered the\n";
  warning_msg += L"track in realtime.\n";
  warning_msg += L"\n";
  warning_msg += L"All tracks that are to be played at the same time must use the\n";
  warning_msg += L"same bailout value. If the bailout values do not match between\n";
  warning_msg += L"tracks in a set, the player will select the lowest value and\n";
  ...
  //   Slider.
  bailout_slider_ = new wxSlider(this, kSlider, 0, 1, 10000,
    wxDefaultPosition, wxSize(200, -1), wxSL_HORIZONTAL | wxCLIP_CHILDREN);
	bailout_sizer->Add(bailout_slider_, 1, wxEXPAND | wxRIGHT, 5);
  //   Text edit.
  bailout_text_ = new wxTextCtrl(this, kText, L"", wxDefaultPosition, wxSize(80, -1));
