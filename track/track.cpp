...
ColorArray TemporalPalette::GetColorArray(const SpatialKeys& spatial_keys, const u32 num_colors) {
  ...
  color_array.reserve(num_colors);
	...
	for (u32 x(0); x < num_colors; ++x) {
		...
		// Find delta position.
		double delta((double)(pos - iter->GetPos()) / (next->GetPos() - iter->GetPos()));
...
void TemporalPalette::CreateTemporalKey(const Pos pos, const SpatialKeys& spatial_keys) {
